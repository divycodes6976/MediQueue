...
const databaseUrl_1 = require("./config/databaseUrl");
...
function logDbTarget(connectionString) {
    try {
...
async function bootstrapDb() {
    ...
    try {
        ...
        const doctors = await pool.query(`SELECT count(*)::int AS n FROM "users" WHERE "role" = 'doctor'`);
...
async function checkDbHealth() {
    const hint = (0, formatError_1.getDatabaseUrlHint)();
    if (!hint.configured) {
        return {
            ok: false,
            databaseUrlConfigured: false,
            databaseHost: null,
            error: "DATABASE_URL is not set on Render. Add Postgres Internal Database URL in Web Service → Environment.",
        };
    }
    try {
        const connectionString = (0, databaseUrl_1.getDatabaseUrl)();
        ...
        try {
            await pool.query("SELECT 1");
            const result = await pool.query(`SELECT count(*)::int AS n FROM "users" WHERE "role" = 'doctor'`);
            return {
                ok: true,
        ...
        return {
            ...
            databaseHost: hint.host,
            error: (0, formatError_1.formatError)(err),
        };
    }
}
