...
import { normalizeQueuePayload } from "@/lib/queue";
import { streamUrl } from "@/lib/api";
import { DEPARTMENTS } from "@/lib/constants";
import { cn } from "@/lib/cn";
const apiPriorityToUi = (p) => {
...
const priorityConfig = {
  EMERGENCY: { icon: AlertTriangle, variant: "danger" },
  SENIOR: { icon: UserCheck, variant: "warning" },
  NORMAL: { icon: User, variant: "info" }
};
function DisplayPage() {
  ...
  return <main className="relative min-h-screen w-full overflow-hidden bg-[#f7f9fd] text-slate-900">
  ...
  >
                  {departments.length === 0 ? <option value="">Loading…</option> : departments.map((dept) => <option key={dept} value={dept}>
                        {dept}
                      </option>)}
                </select>
  ...
  >
          ...
          <Card className="flex flex-col border-slate-200/90 p-6 shadow-[0_2px_8px_rgb(15_23_42_/_0.04)]">
            ...
            <div className="flex-1 space-y-3">
              {nextTokens.length === 0 ? <div className="flex min-h-[106px] flex-col items-center justify-center rounded-xl border border-dashed border-slate-200 bg-slate-50/60 p-5 text-center">
    ...
    >
        ...
        <Card padding="none" className="overflow-hidden border-slate-200/90 shadow-[0_2px_8px_rgb(15_23_42_/_0.04)]">
          ...
          {error && <p className="mx-6 mt-4 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">
              {error}
            </p>}

          {rows.length === 0 ? <div className="flex min-h-[220px] flex-col items-center justify-center p-8 text-center">
            ...
            </div> : <div className="overflow-x-auto p-4 sm:p-6">
