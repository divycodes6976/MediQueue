...
import { apiToUiPriority, DEPARTMENTS } from "@/lib/constants";
import { cn } from "@/lib/cn";
import { normalizeQueuePayload, normalizeQueueToken } from "@/lib/queue";
...
function formatPatientDetails(age, phone) {
  const cleanPhone = phone?.replace(/\D/g, "") ?? "";
  const maskedPhone = cleanPhone ? `\u2022\u2022\u2022\u2022${cleanPhone.slice(-4)}` : null;
  return [age != null ? `${age}y` : null, maskedPhone].filter(Boolean).join(" \xB7 ") || "\u2014";
}
...
function DoctorPage() {
  ...
  const consultStartRef = useRef(null);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    void api.get(waitingUrl(department)).then(({ data }) => {
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      if (consultStartRef.current) {
        setConsultSeconds(Math.floor((Date.now() - consultStartRef.current) / 1e3));
      }
    }, 1e3);
  ...
  const callNext = async () => {
    ...
    try {
      const { data } = await api.post("/queue/call-next");
  ...
  return <div className="space-y-7 pb-5">
  ...
  >
              {DEPARTMENTS.map((item) => <option key={item.code} value={item.code}>
                  {item.code}
                </option>)}
            </Select>
            {sseStatus === "open" ? <Badge variant="success" className="bg-emerald-50/80 px-3 py-1.5 font-medium shadow-[0_1px_2px_rgb(15_23_42_/_0.03)]">
                <span className="h-1.5 w-1.5 rounded-full bg-emerald-500 pulse-ring" />
                <Wifi className="h-3.5 w-3.5" /> SSE Connected
              </Badge> : <Badge variant="outline" className="bg-white/80 px-3 py-1.5 shadow-sm">
  ...
  >
              {nowServing && <div className="absolute -right-8 -top-8 h-32 w-32 rounded-full bg-blue-500/5 blur-2xl" />}
              ...
              <div className="relative z-10 flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
                ...
                {nowServing && nowServingDisplay && nowServingDisplay.ageOrPhone !== "\u2014" && <p className="mt-1 text-sm text-slate-500">{nowServingDisplay.ageOrPhone}</p>}
                {nowServing && <div className="mt-3 flex flex-wrap items-center gap-2">
  ...
  >
              ...
              {error && <p className="relative mt-4 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">
                  {error}
                </p>}
            </Card>
          </motion.div>
      ...
      <Card padding="none">
        ...
        {queueRows.length === 0 ? <div className="p-6">
  ...
  >
            ...
            <div className="space-y-3 p-4 md:hidden">
              {queueRows.map((row, i) => <motion.div
    key={row.token}
